...
RSpec.describe 'Api::Articles::CommentsController', type: :request do
  ...
  describe 'index' do
    ...
    context 'when page is not provided' do
      ...
      it {
        expect(hash_body['comments'][4].to_json)
          .to eq serialize_model(Article.find(article.id).comments_with_user_email[4])
      }
      it {
        expect(hash_body['comments'].none? do |comment_|
                 comment_['id'].to_i == Comment.where(article_id: article.id)[5].id
               end).to be_truthy
      }
    end
  end
